...
export const useLignes = () => {
  ...
  const fetchLignes = useCallback(async (isRefreshing = false) => {
    ...
    try {
      ...
      // FILTRER LES LIGNES ACCEPTÉES POUR CHAQUE DISTRICT
      const filteredDistricts = res.map(district => ({
        ...district,
        lignes: district.lignes.filter(
          ligne => ligne.statut?.toLowerCase() === 'accepted' || 
