...
export class RegionService{
    private regisionRepository = AppDataSource.getRepository(Region);
    ...
    async getRegionById(id:number){
        return this.regisionRepository.findOne(
            {where:{id},relations:["districts","districts.lignes","districts.arrets"]}
        )
    }
     ...
     */
    async createRegion(data:Partial<Region> & {province_id?:number}){
        if(data.province_id){
            data.province = {id:data.province_id} as Province;
