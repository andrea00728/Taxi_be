...
export class ItineraireService{
    private itineraireRepository= AppDataSource.getRepository(Itineraire);
     ...
     */
    async getItinById(id:number){
        return await this.itineraireRepository.findOne(
            {where:{id},
            relations:["ligne"]}
        );
    }
     ...
     */
    async createItineraires(data:Partial<Itineraire>){
    ...
    async updateItin(id:number,data:Partial<Itineraire>){
     ...
     * @returns {Promise<Itineraire[]>} A promise that resolves to an array of Itineraire objects.
     */
    async findByLIgne(ligneId:number){
        return await this.itineraireRepository.find(
